...
export function getToolDefinitions() {
  return [
    ...
    },
    {
      ...
      description:
        'Get per-RPC-provider quality indicators (Infura, QuickNode, Alchemy, dRPC, Chainstack, …) over a ~30-day window: incidents30d, ongoingNow, maintenance30d, chainsAffected30d, resolutionHours (median/avg time-to-resolve), availability, endpointReachability, and chain coverage. For "which provider is most reliable/available" use `availability` ({percent, basis: "status-page-incidents", selfReported: true}) — time-weighted from the incident durations the provider posts on its own status page. It is SELF-REPORTED: a provider that never posts incidents scores a perfect 100 (a silent status page looks perfect), so always say it is self-reported. `endpointReachability` ({working, total, percent, registryChains}) measures whether REGISTRY-LISTED endpoint URLs answer chains-api\'s probes — keyed/stale/geo-blocked registry URLs fail by design, so it is a registry data-quality signal; do NOT present it as provider uptime. `windowDays` is the actually-observed window (may be under 30 when feed retention is shorter).',
      inputSchema: {
        type: 'object',
        properties: {
          provider: {
