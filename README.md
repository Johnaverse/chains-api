...
- `GET /forks?chainId=&phase=&scheduledOnly=&limit=`: forks as ENTITIES — one entry per fork per network, with every provider window grouped under it, so a fork that six providers announced appears once rather than six times. Carries `activationAt` **with** `activationEvidence` (`stated` / `observed` from a reached block height / `estimated` from a future one — approximate, not a schedule), `activationBlock`, `state`, lifecycle `phase` (`upcoming`/`past`/`cancelled`/`unscheduled`) and `sources`. `scheduledOnly=true` for a calendar view; `byPhase` counts cover everything matched, not just the page. Also exposed as the `get_forks` MCP tool.
